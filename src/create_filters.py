...
def create_top_hat_filters(galaxpy_path, params):
  r""" Create top-hat filters

    Inputs:    
      galaxpy_path (string): galaxpy's path      
      params (dict): dictionary with the following keys:
        name_survey: survey name for this set of filters
        lambda_res: spectral resolution R=lambda/Delta_lambda
    ...
    Outputs:
      A set of files named name_survey+'-band_'+n+'.filt', 
      where n is the nth filter. These are ranked according to their
      central wavelength and saved in galaxpy/filters/name_survey/
  ...
  folder_filters = galaxpy_path + '/filters/' + name_survey + '/'
